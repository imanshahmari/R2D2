...
int main() {
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {				// until the Esc is pressed or webcam connection is lost
		...
		cv::HoughCircles(threshImg,v3fCircles,cv::HOUGH_GRADIENT,2,threshImg.rows / 4,100,50,10,100);  // algorithm for detecting circles		
		...
		// declare windows
	    // Create a window
	    static const std::string kWinName1 = "imgOriginal";
	    static const std::string kWinName2 = "threshImg";
