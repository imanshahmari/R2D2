...
std_msgs::Int16MultiArray motorReading_output;
...
ros::Publisher motorReading_publisher("motorReading_topic", &motorReading_output);
...
void setup()
{
  ...
  node_handle.advertise(motorReading_publisher);
...
{ 
  ...
  motorReading_output.data[0] = a_vel;
  motorReading_output.data[1] = b_vel;
  //lineTracking_output.data = digitalRead(4);
  
  //lineTracking_publisher.publish(&lineTracking_output);
  distanceReading_publisher.publish(&distanceReading_output);
  motorReading_publisher.publish(&motorReading_output);
  
  int val;
  
  val = incPulse( servoRoll.read(), 16);
  servoRoll.write(val);
