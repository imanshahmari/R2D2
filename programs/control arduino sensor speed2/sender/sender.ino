...
{ 
  ...
  while(exitVar == false) {
    ...
    if(inChar == 'p' || isDigit(inChar) || inChar == '\r' || inChar == '\n'){
      Serial.println("it is somehting useful");
      inputString += inChar;
    }
    if (inChar == '\n') {
      ...
      controlString.remove(inputString.length()-1);
      Serial.println("control string is:");
      Serial.println(controlString);
      Serial.println("first char is:");
      Serial.println(controlString.charAt(0));
      if (controlString.charAt(0) == 'p') {
