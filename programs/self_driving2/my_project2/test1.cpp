...
int getMostFrequentElement(std::deque<int> &arr)
{
    if (arr.empty())
        return -1;
...
void getSensorDistance(serial::Serial& channel, int& forward_distance, std::deque<int>& latest_distances) {
    ...
    while(running == true){
        ...
        std::cout << "From arduino: "<< message_int << std::endl;
        /*
        coutMutex.lock();
        ...
        for (int i=0; i < 9; ++i){
            std::cout << latest_distances[i]; 
            std::cout << ",";
        }       
        std::cout << std::endl;
        latest_distances.push_back((int)message_int);
        latest_distances.pop_front();


        forwardDistanceMutex.lock();
        forward_distance = getMostFrequentElement(latest_distances);
        //forward_distance = message_int;
        std::cout <<"Freq:"<< forward_distance << std::endl;
...
int main()
{
    ...
    std::deque<int> latest_distances = { 0, 0, 0, 0, 0 ,0 ,0 ,0 ,0};
    ...
    std::thread t1(getSensorDistance,std::ref(my_serial), std::ref(forward_distance),std::ref(latest_distances));
    ...
    */
    while(true){
        if (doneNothing_Finished == false){
            ...
            while(forward_distance > 35 || forward_distance == 0 || forward_distance == 1){
