...
#define forward_speed "m1100090\r\n"
#define backward_speed "m0150150\r\n"
#define left_forward_speed "m1000120\r\n"
#define left_backward_speed "m0000120\r\n"
#define right_forward_speed "m1120000\r\n"
...
void ActuateWithDirection(serial::Serial& my_serial,int& backward_distance, int degree,bool& ActuateWithDirection_Finished,bool& doneNothing_Finished){
    if (degree < 50  && degree > 1){
        //my_serial.flushOutput();
        if(backward_distance > minimum_distance || backward_distance == 0){
            my_serial.write(left_backward_speed);
            std::cout << "left_backward_Active" << std::endl;
        }
        else{
            my_serial.write(right_forward_speed);
            std::cout << "right_forward_Active" << std::endl;
        }
        ...
        if(backward_distance > minimum_distance || backward_distance == 0){
            ...
            std::cout << "right_backward_Active" << std::endl;
        }
        else{
            my_serial.write(left_forward_speed);
            std::cout << "left_forward_Active" << std::endl;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
    }
        ...
        if (doneNothing_Finished == false){
            ...
            if(Scan_Finished == false){
                ...
                if (ActuateWithDirection_Finished==false){
                    ActuateWithDirection(std::ref(my_serial),std::ref(backward_distance), std::ref(degree),std::ref(ActuateWithDirection_Finished),std::ref(doneNothing_Finished));
