...
void GoForward(serial::Serial& my_serial,bool& Scan_Finished, int& forward_distance){
    while(forward_distance > 35  || forward_distance == 0 || forward_distance == 1){
        my_serial.flushOutput();
        my_serial.write("m1000200\r\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(125));
        my_serial.flushOutput();
        my_serial.write("m1200000\r\n");
    ...
    */
    while(true){
        if (doneNothing_Finished == false){
            ...
            if(forward_distance > 35 || forward_distance == 0 || forward_distance == 1){
                GoForward(std::ref(my_serial),std::ref(Scan_Finished),std::ref(forward_distance));
